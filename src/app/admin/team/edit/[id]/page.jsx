...
export default function EditTeamMemberPage({ params }) {
    ...
    useEffect(() => {
        fetch(`/api/team/${id}`)
            ...
            .catch(() => { setError('Failed to load member.'); setStatus('idle'); });
    }, [id]);
            ...
            <nav style={{ background: 'rgba(255,255,255,0.03)', borderBottom: '1px solid rgba(255,255,255,0.07)', padding: '0 32px', height: '60px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div className="d-flex align-items-center gap-3">
                    ...
                    <span style={{ color: '#fff', fontWeight: 600 }}>
                        <i className="bi bi-pencil-fill me-2" style={{ color: '#e879f9' }}></i>Edit Member
                    </span>
                </div>
            </nav>

            <main style={{ padding: '32px', maxWidth: '680px', margin: '0 auto' }}>
                <h1 style={{ color: '#fff', fontSize: '22px', fontWeight: 700, marginBottom: '28px' }}>Edit Team Member</h1>

                {error && (
                    <div style={{ background: 'rgba(247,148,29,0.12)', border: '1px solid rgba(247,148,29,0.25)', borderRadius: '8px', padding: '12px 16px', color: '#F9B45E', marginBottom: '20px', fontSize: '14px' }}>
                    ...
                    <form onSubmit={handleSubmit}>
                        <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '28px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                            ...
                            <div>
                                ...
                                <input name="role" value={form.role} onChange={handleChange} style={inputStyle} required />
                            </div>
                            ...
                            <div>
                                ...
                                <input name="facebook" value={form.facebook} onChange={handleChange} style={inputStyle} />
                            </div>
                            <div>
                                <label style={labelStyle}>Twitter / X URL</label>
                                <input name="twitter" value={form.twitter} onChange={handleChange} style={inputStyle} />
                            </div>
                            <div>
                                <label style={labelStyle}>LinkedIn URL</label>
                                <input name="linkedin" value={form.linkedin} onChange={handleChange} style={inputStyle} />
                            </div>
                            ...
                            <div className="d-flex align-items-center gap-2 mt-2">
                                ...
                                >
                                    {status === 'saving' ? <><i className="bi bi-hourglass-split me-2"></i>Saving…</> : <><i className="bi bi-check-lg me-2"></i>Save Changes</>}
                                </button>
